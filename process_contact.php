...
// Ensure redirect_url is safe (relative path or same domain)
// Simple check: if it starts with http, make sure it's our domain. For now, just trust relative paths or default to contacto.html
if (empty($redirect_url)) {
    $redirect_url = 'contacto.html';
}

if (count($errores) > 0) {
    $_SESSION['errores'] = $errores;
    // Append anchor if not present
    if (strpos($redirect_url, '#contacto') === false) {
