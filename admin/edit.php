...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $db->prepare("
            UPDATE blog_posts 
            SET title=?, slug=?, excerpt=?, content=?, image_path=?, category_id=?, author=?, is_featured=?, status=?
            WHERE id=?
        ");
        $stmt->execute([$title, $slug, $excerpt, $content, $imagePath, $category ?: null, $author, $featured, $status, $id]);
...
<head>
    ...
    <title>Editar Post - Admin Blog</title>
                            ...
                            <div class="form-group">
                                <?php if ($currentImage): ?>
                                    <div class="current-image">
                                        <img src="../<?= htmlspecialchars($currentImage) ?>" alt="Imagen actual">
                                        <label class="remove-image">
                                            <input type="checkbox" name="remove_image" value="1"> Eliminar imagen
                                        </label>
                                    </div>
                                <?php endif; ?>
                                <div class="image-upload-area" id="imageUploadArea">
                                    ...
                                    <div class="upload-placeholder" id="uploadPlaceholder">
                                        ...
                                        <span><?= $currentImage ? 'Cambiar imagen' : 'Click para subir imagen' ?></span>
